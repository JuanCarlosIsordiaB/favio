        ...
        <div className="mb-8 bg-white p-6 rounded-xl shadow-sm border border-slate-200 relative">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Moneda de Gestión</label>
                <select
                  value={formData.management_currency}
                  onChange={(e) => setFormData({ ...formData, management_currency: e.target.value })}
                  className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
                >
                  <option value="UYU">UYU - Peso Uruguayo</option>
                  <option value="USD">USD - Dólar Estadounidense</option>
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Perfil de Contribuyente</label>
                <select
                  value={formData.taxpayer_profile}
                  onChange={(e) => setFormData({ ...formData, taxpayer_profile: e.target.value })}
                  className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
                >
              ...
              <div className="md:col-span-2">
                <label className="block text-sm font-medium text-slate-700 mb-2">Unidades de Negocio</label>
                <div className="space-y-2 p-3 border border-slate-300 rounded-lg bg-slate-50">
                  {['AGRICULTURA', 'GANADERIA', 'LECHERIA', 'MAQUINARIA'].map(unit => (
                    <label key={unit} className="flex items-center gap-3 cursor-pointer">
                      <input
                        type="checkbox"
                        checked={formData.business_units.includes(unit)}
                        onChange={(e) => {
                          const newUnits = e.target.checked
                            ? [...formData.business_units, unit]
                            : formData.business_units.filter(u => u !== unit);
                          setFormData({ ...formData, business_units: newUnits });
                        }}
