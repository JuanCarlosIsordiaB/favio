...
import { crearOrdenPago, aprobarOrdenPago, ejecutarOrdenPago } from '../services/paymentOrders';
import { verificarAlertasFacturas, verificarAlertasIngresos, verificarPagosProgramadosProximos } from '../services/financeAlerts';
...
function FinanceManagerContent({ firmId, premiseId, initialTab = 'expenses' }) {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`expenses-${firmId}`)
      .on(
        'postgres_changes',
        {
        ...
        (payload) => {
          console.log('📡 Cambio detectado en expenses:', payload.eventType, payload.new?.id);
          ...
          console.log('🔔 Triggereando verificación de alertas por cambio en expenses');
          checkFinanceAlerts();
        }
      )
      .subscribe((status) => {
  ...
  useEffect(() => {
    ...
    // Crear canal de Realtime
    const channel = supabase.channel(`incomes-${firmId}`)
      .on(
        'postgres_changes',
        {
          event: '*', // Escuchar INSERT, UPDATE, DELETE
        ...
        (payload) => {
          console.log('📡 Cambio detectado en incomes:', payload.eventType, payload.new?.id);
          // Refrescar incomes cuando hay cambios en la BD
          // Nota: loadIncomesData es estable (useCallback), así que es seguro usarlo aquí
          loadIncomesData(firmId, {});

          // ✨ NUEVO: Triggear verificación de alertas inmediatamente
          console.log('🔔 Triggereando verificación de alertas por cambio en incomes');
          checkFinanceAlerts();
        }
      )
      .subscribe((status) => {
  ...
  useEffect(() => {
    const calculateMetrics = async () => {
      ...
      const totalCuentasPorPagar = expenses
        .filter(e => ['APPROVED', 'PAID_PARTIAL'].includes(e.status) && e.balance > 0)
        .reduce((sum, e) => sum + (e.balance || 0), 0);

      const totalCuentasPorCobrar = incomes
        .filter(i => ['CONFIRMED', 'COLLECTED_PARTIAL'].includes(i.status) && i.balance > 0)
        .reduce((sum, i) => sum + (i.balance || 0), 0);

      const facturasVencidas = expenses.filter(e => {
        if (!e.due_date || ['PAID', 'CANCELLED'].includes(e.status)) return false;
      ...
      const totalAccountsBalance = accounts.reduce((sum, acc) => sum + (acc.current_balance || 0), 0);
  ...
  useEffect(() => {
    console.log('💰 Cuentas cargadas en FinanceManager:', {
      ...
      totalCalculado: accounts.reduce((sum, a) => sum + (a.current_balance || 0), 0)
    });
  }, [accounts]);
  ...
  const handlePaymentOrderSubmit = async (orderData, expensesWithAmounts) => {
    try {
      ...
      const { data: nuevaOrden, error } = await crearOrdenPago(dataToSubmit, expensesWithAmounts);
      ...
      localStorage.setItem(`paymentOrder_${nuevaOrden.id}`, JSON.stringify(relaciones));
      console.log(`💾 Relaciones guardadas en localStorage para orden ${nuevaOrden.id}`);

      // 2. Aprobar la orden automáticamente
      const { data: ordenAprobada, error: errorAprobacion } = await aprobarOrdenPago(nuevaOrden.id, userId);

      if (errorAprobacion) throw errorAprobacion;
      console.log('✅ Orden de pago aprobada:', ordenAprobada.id);
      toast.success('Orden de pago aprobada');

      // 3. Ejecutar la orden para actualizar facturas
      const { data: ordenEjecutada, error: errorEjecucion } = await ejecutarOrdenPago(nuevaOrden.id, userId);

      if (errorEjecucion) throw errorEjecucion;
      console.log('✅ Orden de pago ejecutada:', ordenEjecutada);
      toast.success('Orden de pago ejecutada - Facturas actualizadas');
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-slate-600 mt-1">Gestión completa de finanzas y facturas</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {/* Cuentas por Pagar */}
        <Card className="border-l-4 border-l-red-500" data-testid="metric-accounts-payable">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-slate-600">Cuentas por Pagar</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-red-600" data-testid="metric-accounts-payable-amount">
              UYU {metrics.totalCuentasPorPagar.toLocaleString('es-UY', { maximumFractionDigits: 0 })}
            </div>
            <p className="text-xs text-slate-500 mt-1">Facturas pendientes de pago</p>
          </CardContent>
        </Card>

        {/* Cuentas por Cobrar */}
        <Card className="border-l-4 border-l-green-500" data-testid="metric-accounts-receivable">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-slate-600">Cuentas por Cobrar</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-green-600" data-testid="metric-accounts-receivable-amount">
              UYU {metrics.totalCuentasPorCobrar.toLocaleString('es-UY', { maximumFractionDigits: 0 })}
            </div>
            <p className="text-xs text-slate-500 mt-1">Ingresos pendientes de cobro</p>
          </CardContent>
        </Card>

        {/* Facturas Vencidas */}
        <Card className="border-l-4 border-l-orange-500" data-testid="metric-overdue-invoices">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-slate-600">Facturas Vencidas</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-orange-600" data-testid="metric-overdue-count">{metrics.facturasVencidas}</div>
            <p className="text-xs text-slate-500 mt-1">Requieren atención inmediata</p>
          </CardContent>
        </Card>

        {/* Pagos del Mes */}
        <Card className="border-l-4 border-l-blue-500" data-testid="metric-payments-month">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-slate-600">Pagos del Mes</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-blue-600" data-testid="metric-payments-month-amount">
              UYU {metrics.pagosDelMes.toLocaleString('es-UY', { maximumFractionDigits: 0 })}
            </div>
            <p className="text-xs text-slate-500 mt-1">Pagos realizados este mes</p>
          </CardContent>
        </Card>

        {/* Ingresos del Mes */}
        <Card className="border-l-4 border-l-emerald-500" data-testid="metric-income-month">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-slate-600">Ingresos del Mes</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-emerald-600" data-testid="metric-income-month-amount">
              UYU {metrics.ingresosDelMes.toLocaleString('es-UY', { maximumFractionDigits: 0 })}
            </div>
        ...
        <Card className="border-l-4 border-l-purple-500" data-testid="metric-accounts-balance">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-slate-600">Balance Cuentas</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-purple-600" data-testid="metric-accounts-balance-amount">
              UYU {metrics.totalAccountsBalance.toLocaleString('es-UY', { maximumFractionDigits: 0 })}
            </div>
            <p className="text-xs text-slate-500 mt-1">Saldo total en cuentas</p>
          </CardContent>
        </Card>
      </div>
      ...
      {!firmId ? (
        <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 text-center">
          <p className="text-amber-800">Selecciona una firma para ver la información financiera</p>
        </div>
      ) : (
        <Tabs value={currentTab} onValueChange={setCurrentTab} className="w-full" data-testid="finance-tabs">
          <TabsList className="grid w-full grid-cols-9" data-testid="finance-tabs-list">
            <TabsTrigger value="expenses" className="flex items-center gap-2" data-testid="tab-expenses">
              <FileText size={16} />
              <span className="hidden sm:inline">Facturas</span>
            </TabsTrigger>
            <TabsTrigger value="income" className="flex items-center gap-2" data-testid="tab-income">
              <TrendingUp size={16} />
              <span className="hidden sm:inline">Ingresos</span>
            </TabsTrigger>
            <TabsTrigger value="purchase_orders" className="flex items-center gap-2" data-testid="tab-purchase-orders">
              <ShoppingCart size={16} />
              <span className="hidden sm:inline">Compras</span>
            </TabsTrigger>
            <TabsTrigger value="payment_orders" className="flex items-center gap-2" data-testid="tab-payment-orders">
              <CreditCard size={16} />
              <span className="hidden sm:inline">Pagos</span>
            </TabsTrigger>
            <TabsTrigger value="accounts_payable" className="flex items-center gap-2" data-testid="tab-accounts-payable">
              <AlertTriangle size={16} />
              <span className="hidden sm:inline">Por Pagar</span>
            </TabsTrigger>
            <TabsTrigger value="accounts_receivable" className="flex items-center gap-2" data-testid="tab-accounts-receivable">
              <DollarSign size={16} />
            ...
            <TabsTrigger value="cost-config" className="flex items-center gap-2" data-testid="tab-cost-config">
              <Settings size={16} />
              <span className="hidden sm:inline">Config. Costos</span>
            </TabsTrigger>
            <TabsTrigger value="event-costs" className="flex items-center gap-2" data-testid="tab-event-costs">
              <BarChart3 size={16} />
          ...
          {/* Facturas de Compra */}
          <TabsContent value="expenses" className="mt-6" data-testid="tab-content-expenses">
            <ExpenseListView
              firmId={firmId}
              onAdd={() => {}}
            />
          ...
          <TabsContent value="income" className="mt-6" data-testid="tab-content-income">
            <IncomeListView
              firmId={firmId}
              onAdd={() => {}}
            />
          ...
          {/* Órdenes de Compra */}
          <TabsContent value="purchase_orders" className="mt-6" data-testid="tab-content-purchase-orders">
            <PurchaseOrders firmId={firmId} />
          </TabsContent>

          {/* Órdenes de Pago */}
          <TabsContent value="payment_orders" className="mt-6" data-testid="tab-content-payment-orders">
          ...
          {/* Cuentas por Pagar */}
          <TabsContent value="accounts_payable" className="mt-6" data-testid="tab-content-accounts-payable">
            <AccountsPayableView
              firmId={firmId}
          ...
          {/* Cuentas por Cobrar */}
          <TabsContent value="accounts_receivable" className="mt-6" data-testid="tab-content-accounts-receivable">
          ...
          {/* Cuentas Financieras */}
          <TabsContent value="accounts" className="mt-6" data-testid="tab-content-accounts">
          ...
          <TabsContent value="cost-config" className="mt-6" data-testid="tab-content-cost-config">
            <EventCostConfigManager firmId={firmId} />
          </TabsContent>

          {/* Costos de Eventos - Generados y Conversión a Facturas */}
          <TabsContent value="event-costs" className="mt-6" data-testid="tab-content-event-costs">
