...
export default function RemittanceReceiveModal({
  ...
  const totalOrdered = itemsData.reduce(
    (sum, item) => sum + (item.quantity_ordered || 0),
    0,
  );
  const totalReceived = itemsData.reduce(
      ...
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-6 p-4">
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <Card>
              <CardContent className="pt-6">
                <div className="text-sm text-slate-600">Total Entregado</div>
            ...
            <Card>
              <CardContent className="pt-6">
                ...
                <div
                  className={`text-2xl font-bold mt-1 ${percentage === 100 ? "text-green-600" : percentage < 100 ? "text-orange-600" : "text-red-600"}`}
                >
                  {percentage}%
                </div>
              </CardContent>
            ...
            <Table className="text-xs">
              ...
              <TableBody>
                {itemsData.map((item) => (
                  <TableRow
                    key={item.id}
                    className={errors[item.id] ? "bg-red-50" : ""}
                  >
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 flex gap-3">
            ...
            <div className="text-sm text-blue-900">
              <strong>Importante:</strong> Al confirmar esta recepción, se
              actualizará automáticamente el stock de los insumos vinculados.
            </div>
          </div>
