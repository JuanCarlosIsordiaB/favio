...
export default function InputFormModal({
  ...
  useEffect(() => {
    if (isOpen && isEditing && insumo) {
      setFormData({
        ...
        stock_status: insumo.stock_status || "disponible",
      ...
      setFormData({
        ...
        expiration_date: "",
        entry_date: new Date().toISOString().split("T")[0],
        stock_status: initialData?.stock_status || "disponible",
  ...
  async function handleSubmit(e) {
    ...
    if (!formData.depot_id || formData.depot_id.trim() === "") {
      setErrors((prev) => ({
        ...prev,
        depot_id: "Debes seleccionar un depósito/ubicación",
      }));
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
            ...
            <div>
              ...
              {depots.length === 0 && (
                <p className="text-yellow-600 text-xs mt-1">
                  ⚠️ No hay depósitos disponibles. Crea lotes con activando la
                  opcion de deposito primero.
                </p>
              )}
              <p className="text-xs text-slate-500 mt-1">
                Ubicación física donde se almacena el insumo
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              {!isEditing && (
                <p className="text-xs text-slate-500 mt-1">
                  Cantidad inicial que se registrará como entrada
                </p>
              )}
              {isEditing && (
                <p className="text-xs text-amber-600 mt-1">
                  ⚠️ No se puede cambiar (usa movimientos para ajustar stock)
                </p>
              )}
            </div>
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1">
                Se generará alerta cuando stock sea menor o igual
              </p>
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={handleInputChange}
                  className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition text-sm bg-white"
                  disabled={isSubmitting}
                >
            ...
            <div>
              {isEditing && onDelete && (
                <button
                  ...
                  title={
                    !canDelete
                      ? "No tienes permiso para eliminar insumos"
