...
export function parsePaymentTerms(paymentTerms, totalAmount, baseDate) {
  ...
  switch (paymentTerms) {
    ...
    default:
      ...
      console.warn(`Condición de pago desconocida: ${paymentTerms}. Usando contado.`);
      cuotas.push({
        installmentNumber: 1,
        percentage: 100,
        daysOffset: 0,
 ...
 */
export async function generateExpensesFromPurchaseOrder(purchaseOrder, firmId, userId) {
  try {
    ...
    if (!purchaseOrder.payment_terms || purchaseOrder.payment_terms === 'contado' || purchaseOrder.payment_terms === '') {
      console.log('⏭️ No se generarán expenses (contado o sin términos)');
      return [];
    }

    // Parsear condiciones de pago
    console.log('📝 [paymentScheduler] Parseando payment_terms:', purchaseOrder.payment_terms);
    ...
    console.log(`✅ [paymentScheduler] Generando ${cuotas.length} expenses programadas para OC ${purchaseOrder.order_number}`, {
      cuotas: cuotas.map(c => ({ installment: c.installmentNumber, amount: c.amount, dueDate: c.dueDate }))
    });
    ...
    if (insertError) {
      ...
      throw new Error(`Error al insertar expenses: ${insertError.message}${insertError.details ? ' - ' + insertError.details : ''}`);
    }

    console.log(`✅ [paymentScheduler] ${createdExpenses.length} expenses creadas exitosamente`);
    ...
    try {
      ...
      console.warn('Warning: Error en auditoría de generación de pagos', auditError);
      // No fallar si hay error en auditoría
    }

    return createdExpenses;
  } catch (error) {
    console.error('❌ [paymentScheduler] Error en generateExpensesFromPurchaseOrder:', {
