...
export async function crearOrdenPago(ordenData, facturasSeleccionadas) {
  try {
    ...
    const totalAmount = facturasSeleccionadas.reduce((sum, f) => sum + (f.amount_paid || f.balance), 0);
    ...
    // Preparar relaciones AHORA para guardarlas en la orden
    const relaciones = facturasSeleccionadas.map(f => ({
      expense_id: f.id,
      amount_paid: f.amount_paid || f.balance
    }));
...
export async function aprobarOrdenPago(id, userId) {
  try {
    ...
    const approvedByValue = (userId === 'sistema' || !isValidUUID) ? null : userId;
...
export async function rechazarOrdenPago(id, userId, motivo) {
  try {
    // Usar NULL para sistema, o el userId si es válido
    const isValidUUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(userId);
    const rejectedByValue = (userId === 'sistema' || !isValidUUID) ? null : userId;
    ...
    console.log('REJECT UPDATE result:', { data, error: updateError, orderId: id });
...
export async function ejecutarOrdenPago(ordenId, userId) {
  try {
    ...
    const { data: orden, error: errorOrden } = await obtenerOrdenPagoPorId(ordenId);
    if (errorOrden) throw errorOrden;
    if (!orden) throw new Error('Orden de pago no encontrada');

    if (orden.status !== 'APPROVED') {
      throw new Error(`La orden debe estar en estado APPROVED. Estado actual: ${orden.status}`);
    }
    ...
    try {
      ...
      if (storedData) {
        ...
        console.log(`📦 Relaciones obtenidas de localStorage: ${relaciones.length} factura(s)`);
      } else {
        console.warn('⚠️ No hay datos de facturas en localStorage para esta orden');
        relaciones = [];
      }
    } catch (e) {
    ...
    for (const relacion of relaciones) {
      ...
      if (errorFactura) {
        console.error(`[ERROR] SELECT failed para expense_id ${relacion.expense_id}:`, errorFactura);
        throw errorFactura;
      }

      console.log(`[DEBUG] Factura encontrada:`, JSON.stringify(factura));

      const montoPagado = (factura.paid_amount || 0) + (relacion.amount_paid || 0);
      const nuevoBalance = (factura.balance || 0) - (relacion.amount_paid || 0);
      const nuevoStatus = nuevoBalance <= 0 ? 'PAID' : 'PAID_PARTIAL';

      console.log(`  💰 Factura ${relacion.expense_id}: pagado=${montoPagado}, balance=${nuevoBalance}, status=${nuevoStatus}`);
      console.log(`[DEBUG] UPDATE payload:`, JSON.stringify({
      ...
      if (errorUpdate) {
        console.error(`[ERROR] UPDATE failed para expense_id ${relacion.expense_id}:`, JSON.stringify(errorUpdate));
    ...
    if (orden.account_id) {
      ...
      console.log(`  🏦 Cuenta ${orden.account_id}: nuevo balance = ${nuevoBalance}`);
    }

    // 6. Actualizar la orden a estado EXECUTED
    // Usar NULL para sistema, o el userId si es válido
    const isValidUUID2 = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(userId);
    const executedByValue = (userId === 'sistema' || !isValidUUID2) ? null : userId;
...
export async function anularOrdenPago(id, userId, motivo) {
  try {
    // Usar NULL para sistema, o el userId si es válido
    const isValidUUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(userId);
    const cancelledByValue = (userId === 'sistema' || !isValidUUID) ? null : userId;
    ...
    console.log('CANCEL UPDATE result:', { data, error: updateError, orderId: id });
