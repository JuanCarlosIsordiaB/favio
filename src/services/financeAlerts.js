...
export async function verificarAlertasFacturas(firmId) {
  try {
    ...
    for (const factura of facturas || []) {
      ...
      else if (diasRestantes > 0 && diasRestantes <= (factura.alert_days || 5)) {
        // Verificar si ya existe alerta similar usando expense_id (FK explícito)
        const { data: alertaExistente } = await supabase
          .from('alerts')
          .select('id')
...
export async function verificarPagosProgramadosProximos(firmId) {
  try {
    const hoy = new Date();
    const fechaHoyStr = hoy.toISOString().split('T')[0];
    const fechaHoy7DiasStr = new Date(hoy.getTime() + 7 * 24 * 60 * 60 * 1000).toISOString().split('T')[0];
...
export async function verificarTodasLasAlertas(firmId) {
  try {
    ...
    const { alertasCreadas: ordenes } = await verificarOrdenesPendientes(firmId);
    resultados.ordenes = ordenes;

    const { alertasCreadas: pagosProgramados } = await verificarPagosProgramadosProximos(firmId);
    resultados.pagosProgramados = pagosProgramados;

    resultados.total = (facturas || 0) + (ingresos || 0) + (ordenes || 0) + (pagosProgramados || 0);
